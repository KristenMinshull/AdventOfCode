...
var orderedWinningScores = new List<int>();
...
foreach (var bingoNumber in bingoNumbers)
{
    foreach (var bingoBoard in bingoBoards.ToList())
    {
        ...
        var winningScore = unmarkedNumbers.Sum() * bingoNumber;
        orderedWinningScores.Add(winningScore);
        bingoBoards.Remove(bingoBoard);
    }
}

Console.WriteLine($"First bingo winner scored {orderedWinningScores.First()}");
Console.WriteLine($"Last bingo winner scored {orderedWinningScores.Last()}");
